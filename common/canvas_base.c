                                       ...
                                       int want_original)
{
    ...
    if (quic_decode_begin(quic_data->quic,
                          SPICE_REALIGN_CAST(uint32_t *,image->u.quic.data->chunk[0].data),
                                 ...
                                 int height)
{
    if (stride_pixman > stride_encoded) {
        ...
        for (row = height - 1; row > 0; --row) {
            ...
            dest_aligned = SPICE_REALIGN_CAST(uint32_t *,dest + stride_pixman*row);
            dest_misaligned = SPICE_REALIGN_CAST(uint32_t*,dest + stride_encoded*row);
...
static int quic_usr_more_space(QuicUsrContext *usr, uint32_t **io_ptr, int rows_completed)
{
    ...
    *io_ptr = SPICE_REALIGN_CAST(uint32_t *,quic_data->chunks->chunk[quic_data->current_chunk].data);
                               ...
                               SpiceQMask *mask, int x, int y)
{
    ...
    mask_data = SPICE_REALIGN_CAST(uint32_t *,(uint8_t *)mask_data + mask_stride * extents.y1 + extents.x1 / 32);
    mask_x -= extents.x1;
    mask_y -= extents.y1;
